...
1. **Wire-protocol versions are lockstep.** `WIRE_PROTOCOL_VERSION` is a `z.literal` in `packages/foundation/schema/src/wire/message.ts` (read the value there — it is deliberately not repeated here); any change to any wire variant must bump it. Mismatched peers still complete the socket, and every frame then fails validation and is **silently discarded** at the transport. A *fresh* `LinkCodeClient` fails closed, because its opening `ping` never draws a `pong` — after 5s the handshake rejects with an error naming the mismatch. Everything else is the silent case: frames dropped mid-session, or after a daemon restarts under a new version. Rebuild and restart the daemon and every client together.
