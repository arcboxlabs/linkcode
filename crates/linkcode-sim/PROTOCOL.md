...
`codec: h264` switches the stream to hardware H.264 (VideoToolbox): the retained framebuffer `IOSurface` is wrapped in a `CVPixelBuffer` and encoded on the media engine — pixels never enter CPU memory (`quality` is ignored: bitrate carries the bandwidth budget, ~10-25× below the JPEG stream; `scale` sizes the compression session down and VideoToolbox scales into it, sides rounded to even for 4:2:0 chroma). Access units are Annex-B with SPS/PPS prepended on keyframes (≤2s apart) — exactly what a WebCodecs `VideoDecoder` configured without a `description` consumes. Delivery is **ordered and lossless** (deltas depend on each other); if the sidecar must drop (stalled consumer), it drops through the next keyframe. If the private worker gives up, the stream degrades to slow simctl JPEG `STREAM_FRAME`s — each wire frame declares its own encoding, so a mixed stream stays decodable.
