...
import type { PanelTabContentItem } from './panel-content-stack';
...
/** One terminal sub-tab's already-rendered content, keyed by tab id. */
export interface SectionTerminalTabContent {
  id: string;
...
export interface SectionPanelRegionProps {
  ...
  /** One entry per `panel.terminal.tabs`, in the same order, with that tab's rendered content. */
  terminalTabs: SectionTerminalTabContent[];
 ...
 * instances, which stay mounted (visibility toggled) across section switches so a running shell
 * survives navigating to Diff and back.
 */
export function SectionPanelRegion({
  ...
  const terminalItems: PanelTabContentItem[] = terminalTabs.map(({ id, node }) => ({
    id,
    active: activeIsTerminal && id === panel.terminal.activeTabId,
    node,
  }));
    ...
    <div className="flex h-full min-h-0 flex-col" style={contentStyle}>
      ...
      <div className="relative min-h-0 flex-1">
        <div className="absolute inset-0">
          <PanelTabContentStack items={terminalItems} />
        </div>
