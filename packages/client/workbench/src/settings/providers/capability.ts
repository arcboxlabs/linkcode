 ...
 * hardcodes nothing. Facts as of 2026-07 (packages/host/agent-adapter `credential.ts` + native
...
export function bindingAvailability(account: Account, kind: AgentKind): BindingAvailability {
  ...
  // An account that DEFINES its provider (endpoint + model list) is injected wholesale at start —
  // pi via registerProvider, opencode via the spawn config (CODE-325) — native regardless of
  // protocol (the api/npm mapping covers all three). Other agents ignore the model list and keep
  // the plain protocol rules on the endpoint.
  if ((kind === 'pi' || kind === 'opencode') && account.customProvider) return { tier: 'native' };
