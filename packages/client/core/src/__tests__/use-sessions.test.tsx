...
class ListingTransport implements Transport {
  ...
  send(message: ValidatedWireMessage): void {
    ...
    if (payload.kind !== 'session.start') return;
    // The daemon registers the record — announcing it — before it answers the request.
    const sessionId = `sess-${this.sessions.length + 1}` as SessionId;
    this.sessions = [...this.sessions, session(sessionId, this.sessions.length + 1)];
    queueMicrotask(() => {
      this.receive({ kind: 'session.changed', sessionId, reason: 'created' });
      this.receive({ kind: 'session.started', replyTo: payload.clientReqId, sessionId });
    });
  }
...
it('drops a session it started once another client closes it', async () => {
  const transport = new ListingTransport();
  const { result } = await connectedHook(transport);

  await act(async () => {
    await result.current.create({ kind: 'claude-code', cwd: '/repo' });
  });
  expect(result.current.sessions.map((s) => s.sessionId)).toEqual(['sess-1']);
