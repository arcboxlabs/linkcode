...
export class OpenCodeAdapter extends BaseAgentAdapter {
  ...
  protected async onStart(opts: StartOptions): Promise<void> {
    ...
    // Catalog fetch is best-effort (no SSE event for catalog changes exists): a failed list must
    // not fail session start — catalog absence is itself the capability signal.
    try {
  ...
  /** Resolves (and caches) the agent name `session.shell` runs under: the session's primary agent,
   * mirroring the upstream TUI's shell dispatch. Any failure to read the catalog bottoms out at
  ...
  // No onSetModel override on purpose: this.model() re-derives from this.opts?.model each prompt,
  // so the mechanism LOOKS live-switchable, but that has never been verified against a real
  // provider (claude-code's identical code-read was wrong once). Rejects via base until verified live.
