...
- **Model catalog** (CODE-104): `listModels()` reads the CLI's own entitlement-aware registry off the `initialize` control response (`Query#supportedModels()`), via a transient never-prompted `Query` closed right after the catalog arrives — `initialize` fires at `Query` construction in streaming mode, no prompt needed. The engine serves it on the pull wire resource `agent-model.list`, cached per boot. The catalog mirrors the CLI's `/model` menu, NOT a per-entry live-switch verification: a listed model can still fail at `setModel` time (fable-5 hard-errors without account access) or silently substitute (opus-4-1 → opus-4-8, observed 2026-07) — vendor behavior, deliberately not filtered locally.
