...
/**
 * Provider × account settings editor. A transport-backed container: it reads/writes the account
 * pool and per-agent config over the data plane (`config.get` / `config.set` via the sdk), so it
 * must render inside `WorkbenchProviders` — but it may sit above the connection gate (both apps
 * mount it in their Settings surface), where it degrades to loading/error while the daemon is down.
 * Shared verbatim by the webview settings route and the desktop settings overlay.
 */
export function AgentAccountsSettings(): React.ReactNode {
  const t = useTranslations('settings.agents');
