 ...
 * - amp authenticates to its own service with `AMP_API_KEY` (not a vendor protocol), so no catalog
 *   protocol backs it — it binds only through a bare-key account (the `protocol === undefined` path).
 */
