...
describe('Composer directive chips', () => {
  ...
  it('leaves modified horizontal arrows to native selection handling', async () => {
    ...
    for (let i = 0, len = modifiedArrowKeys.length; i < len; i++) {
      const [key, modifier] = modifiedArrowKeys[i];
  ...
  it('disables chip and recovery actions with the composer', () => {
    ...
    const statusActions = within(screen.getByRole('status')).getAllByRole<HTMLButtonElement>(
      'button',
    );
    for (let i = 0, len = statusActions.length; i < len; i++) {
