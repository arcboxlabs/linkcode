...
export class ClaudeCodeAdapter extends BaseAgentAdapter {
  ...
  protected async onStart(opts: StartOptions): Promise<void> {
    ...
    // default with an error event rather than failing session creation. (`opts.effort` needs no
    // mirror here — base `start()` routes it through `onSetEffort` before `onStart`.)
