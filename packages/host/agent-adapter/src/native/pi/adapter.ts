...
export class PiAdapter extends BaseAgentAdapter {
  ...
  /** Retry-aware turn finalization (CODE-342): pi emits an intermediate `agent_end {willRetry}`
   * per attempt and a single `agent_settled` when the whole turn (all retries) is done. The turn
   * is only settled on `agent_settled`; `agent_end` just records the latest outcome. */
  private turnActive = false;
  private promptInFlight = false;
  private settlementPending = false;
  private finalOutcome: { kind: 'error'; message: string } | { kind: 'stop'; reason: StopReason } =
  ...
  /** Initial effort stashed by the pre-`onStart` `onSetEffort` call (base routes `opts.effort`
   * through the hook before the session exists); `onStart` applies it at session creation. */
  private pendingEffort: PiEffort | undefined;
  ...
  protected async onStart(opts: StartOptions): Promise<void> {
    ...
    // Base already routed `opts.effort` through `onSetEffort` before this hook; a valid initial
    // level is stashed in `pendingEffort`, an unsupported one degraded there with an error event.
    const initialThinking = this.pendingEffort;
  ...
  protected async onPrompt(content: ContentBlock[]): Promise<void> {
    ...
    this.finalOutcome = { kind: 'stop', reason: 'end_turn' };
    ...
    try {
      ...
      // A retry runs while `prompt()` is still pending; the turn only ends once `agent_settled`
      // fires (CODE-342). If it already arrived, settle now that the prompt promise has resolved.
  ...
  protected override onSetEffort(effort: EffortLevel): Promise<void> {
    ...
    if (!isPiEffort(effort)) {
      // Live switch: reject so the composer keeps the prior level. Initial pick (pre-session, base
      // routes `opts.effort` here before `onStart`): degrade with an error event so a stale
      // cross-provider effort never bricks session creation.
      const message = `pi: effort '${effort}' is not supported (low–xhigh only)`;
      if (!session) {
    ...
    // Pre-session: stash for `onStart` to hand `createAgentSession`, which clamps to the model.
    if (!session) {
  ...
  protected override async onCancel(): Promise<void> {
    // Record the intent so the eventual `agent_settled` (which may trail a retry backoff) maps to
    // a cancel rather than the last attempt's outcome.
    this.finalOutcome = { kind: 'stop', reason: 'cancelled' };
  ...
  protected handleEvent(ev: AgentSessionEvent): void {
    switch (ev.type) {
      case 'agent_start':
        ...
        this.turnActive = true;
        // Reset the outcome so a prior turn's cancel/error never leaks into this one (the
        // agent_end cancel-guard only means an in-turn onCancel — see below).
        this.finalOutcome = { kind: 'stop', reason: 'end_turn' };
        this.freshSegment();
        ...
        break;
      case 'agent_end': {
        // Per-attempt outcome only — an aborted/failed run still ends with agent_end, carrying a
        // final assistant message whose stopReason is 'aborted'/'error' (+ errorMessage); pi emits
        // no separate error event. A retry (`willRetry`) means another attempt follows, so the turn
        // is settled on agent_settled, not here (CODE-342). A prior cancel intent wins.
        if (this.finalOutcome.kind === 'stop' && this.finalOutcome.reason === 'cancelled') break;
        ...
        if (outcome === 'error') {
          ...
          this.finalOutcome = { kind: 'error', message: `pi: ${detail}` };
        } else {
          this.finalOutcome = { kind: 'stop', reason: piStopReason(outcome) };
        }
        break;
      }
      case 'agent_settled':
        // The whole turn (all retries) is done. Defer if the prompt promise is still pending so
        // `onPrompt` settles after it resolves — otherwise the settle races the returning await.
  ...
  /** Finalize the turn once `agent_settled` reports the whole run (all retries) is done: sweep
   * liveness and emit the recorded outcome. A failed turn emits an error and no stop event (the
   * opencode failed-turn contract); success/cancel/length emit the mapped stop. */
  private settleTurn(): void {
    if (!this.turnActive) return;
    this.turnActive = false;
    this.settlementPending = false;
    this.teardown();
    if (this.finalOutcome.kind === 'error') this.emitError(this.finalOutcome.message);
    else this.emitStop(this.finalOutcome.reason);
    this.emitStatus('idle');
  }
}
