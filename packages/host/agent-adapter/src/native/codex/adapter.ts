...
export class CodexAdapter extends BaseAgentAdapter {
  ...
  protected async onStart(opts: StartOptions): Promise<void> {
    ...
    // An initial approval-policy pick from the new-session surface rides the next `turn/start`
    // like its live switch; an invalid tier degrades with an error event rather than failing
    // session creation. (`opts.effort` needs no mirror — base routes it through `onSetEffort`.)
