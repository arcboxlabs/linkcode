...
describe('pi turn lifecycle', () => {
  ...
  it('maps agent_end stop reasons and surfaces run errors at settlement', async () => {
    const { adapter: _adapter, session, events } = await startedAdapter();

    // Each turn records its outcome on `agent_end` and settles on `agent_settled` (CODE-342).
    session.feed({ type: 'agent_start' });
    session.feed({ type: 'agent_end', messages: [{ role: 'assistant', stopReason: 'stop' }] });
    session.feed({ type: 'agent_settled' });
    ...
    // agent_start emits running, settlement emits idle.
    expect(statuses(events)).toEqual(['running', 'idle']);
  });
});
