...
export class ConversationProjectionService {
  ...
  /** Read each turn from its executing run, never a fork's re-stamped copy. If unavailable,
   * replay its complete retained journal interval or show a placeholder. */
  private composeDurable(
    ...
    const { checkpoints, records, turns, journals } = this;
    return Effect.gen(function* () {
