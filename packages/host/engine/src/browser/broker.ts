...
export class BrowserBrokerService {
  ...
  dispatch(op: BrowserOp, args: BrowserCommandArgs): Promise<BrowserCommandResult> {
    ...
    return new Promise((resolve) => {
      ...
      try {
        ...
        ).catch(() => this.failDispatch(commandId));
      } catch {
        this.failDispatch(commandId);
      }
    });
  }
  ...
  private failDispatch(commandId: string): void {
    const pending = this.pending.get(commandId);
    if (!pending) return;
    this.pending.delete(commandId);
    clearTimeout(pending.timer);
    pending.resolve(failure('host-unavailable', 'the browser host connection is closed'));
  }
