...
describe('watchTurn', () => {
  it('concatenates chunks by messageId and resolves on stop', async () => {
    ...
    const result = watchTurn(a, () => a.send({ type: 'prompt', content: [textBlock('go')] }));
  ...
  it('ignores a recoverable error but rejects on a fatal one', async () => {
    ...
    const ok = watchTurn(recoverable, asyncNoop);
    await Promise.resolve();
    ...
    const bad = watchTurn(fatal, asyncNoop);
    await Promise.resolve();
  ...
  it('rejects and cancels on timeout', async () => {
    ...
    try {
      const a = new WatchAdapter();
      const result = watchTurn(a, asyncNoop, { timeoutMs: 1000 });
