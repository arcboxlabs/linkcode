...
class RejectingTransport extends FakeTransport {
  override send(msg: WireMessage): Promise<void> {
    this.sent.push(msg);
    return msg.payload.kind === 'browser.command'
...
describe('BrowserBrokerService', () => {
  ...
  it('settles immediately when transport send rejects', async () => {
    ...
    expect(result).toEqual({
      ...
      error: {
        ...
        message: 'the browser host connection is closed',
        retryable: true,
      },
    });
