...
- Tooling exception: this package is **excluded from ESLint and from the root Biome config** (`!packages/coss-ui` in `biome.json`), so upstream formatting is preserved as-synced — never reformat it.
