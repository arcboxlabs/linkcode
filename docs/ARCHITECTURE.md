...
coding agents — Claude Code, Codex, OpenCode, Pi, and Amp — and normalizes their
...
| `agent-adapter` | One adapter per agent (`claude-code` / `codex` / `opencode` / `pi` / `amp`) plus the abstraction layer that normalizes native events into `schema`. |
...
`amp` — each hiding its SDK's differences behind the unified `AgentAdapter` interface. A shared
...
interface AgentAdapter {
  readonly kind: AgentKind;                                  // 'claude-code' | 'codex' | 'opencode' | 'pi' | 'amp'
...
| Agents                 | Claude Code · Codex · OpenCode · Pi · Amp                                     |
...
| ACP               | Agent Client Protocol — an early influence on `AgentEvent`, since dropped for a shape tailored to the supported agents. |
