...
| State, workspace, and data dirs | `packages/foundation/schema/src/product.ts`: the `BRAND` / `STATE_BRAND` constants behind `stateDirBasename()` (`~/.linkcode`), `workspacesDirName()` (`~/LinkCode`), `dataDirName()` / `xdgDataDirName()` (managed-asset store under the platform data dir) | One edit rebrands every on-disk footprint — daemon state, workspace root, asset store — across daemon, desktop, and adapters. Each path already forks by channel (release vs development, CODE-460), but **not** by product: an unrenamed fork still shares `~/.linkcode` and `~/LinkCode` with an installed LinkCode of the same channel, and one daemon per state universe means the two would fight over the universe and its port range (release `19523`–`19532`, development `19533`–`19542`; `/linkcode` identity probe). Rename these — and ideally `DAEMON_DEFAULT_PORT` and the identity path in `daemon-runtime.ts` — if your fork should coexist with LinkCode on one machine. |
| Cloud endpoints | `DEFAULT_CLOUD_URL = https://api.linkcode.ai` (`apps/daemon/src/cloud/api.ts`); `CLOUD_API_URL` / sign-in URL (`apps/desktop/src/main/cloud-auth/client.ts`, overridable via `LINKCODE_CLOUD_API_URL` / `LINKCODE_CLOUD_SIGN_IN_URL`); mobile equivalents under `apps/mobile/src/runtime/cloud` | Cloud login is opt-in — a fork is fully functional local-first without it. Point these at your own deployment or leave the feature dormant; do not ship a fork that signs users into ArcBox's Cloud under your brand. |
| Daemon credential vault | the OS-keyring service name comes from `keyringServiceName()` in `daemon-runtime.ts`, which is `dataDirName()` — so the same `BRAND` edit above renames it | A fork that renames the brand automatically gets its own keyring entries; one that does not will read and overwrite an installed LinkCode's daemon secrets on the same machine. |
...
| Package feeds | Homebrew cask in `arcboxlabs/homebrew-tap`; WinGet package `ArcBox.LinkCode` in `microsoft/winget-pkgs`; the R2 release feed | ArcBox's distribution channels. Publish your fork through your own tap/feeds and your own package identifier. |

## What already fails safe

- **Signing and publishing are ArcBox-maintainers-only** and live behind the `release` GitHub environment: macOS Developer ID + notarization, Windows Trusted Signing (OIDC), the R2 feed upload, the Homebrew cask bump, and the WinGet bump all no-op without its secrets — a fork's CI cannot reach them.
