...
| State and workspace dirs | `~/.linkcode` (daemon state) and `~/LinkCode` (workspace root) in `apps/daemon/src/config.ts` | Shared with an installed LinkCode, and the daemon is one-per-machine (port `19523`, `/linkcode` identity probe) — rename the dirs (and ideally the port/identity path) if your fork should coexist with LinkCode on the same machine. |
