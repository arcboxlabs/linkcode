...
| `SENTRY_AUTH_TOKEN` | `build-mobile.yml` | Uploads production mobile source maps. Local EAS Build cannot read an EAS variable with Secret visibility, so GitHub must inject it. |
| `SENTRY_DSN_MOBILE`, `POSTHOG_PROJECT_TOKEN`, `POSTHOG_HOST` | `build-mobile.yml` | Mapped to the mobile `EXPO_PUBLIC_*` build-time variables. These are publishable identifiers, but the repository currently carries them as Actions secrets. |
