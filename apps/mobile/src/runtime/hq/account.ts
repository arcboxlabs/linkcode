...
import { hqAuthClient } from './client';
 ...
 */
export function useHqAccount(): HqAccount {
  const { data, isPending } = hqAuthClient.useSession();
...
export async function signOutOfHq(): Promise<void> {
  await hqAuthClient.signOut();
