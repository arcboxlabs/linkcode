...
/** The host's three top-level surfaces. `NativeTabs` is a real `UITabBarController`, so the iOS 26
 * floating tab bar and its scroll-minimize behaviour come from UIKit rather than being drawn here.
 *
 * Detail screens (a thread, a terminal) deliberately live outside this layout: react-native-screens
 * exposes `tabBarHidden` only on the host, not per pushed screen, so the only way to keep the bar
 * off a composer or a terminal canvas is to push those over the tabs from the host stack. */
export default function HostTabsLayout(): React.ReactNode {
