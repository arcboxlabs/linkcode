...
- **Terminal canvas:** the RN route owns `LinkCodeClient`, attachment/controller state, and all
  ...
  The package's `postinstall` downloads the checksum-pinned GhosttyKit.xcframework — it must stay
  in root `allowBuilds:`, and adding/upgrading it changes the native fingerprint (new dev build).
